...
class SocialShare implements Lib\ShortcodeInterface {
	...
	private function getSocialNetworkShareLink($net, $image) {
        $net_image = is_array($image) ? $image[0] : '';
		switch ($net) {
			case 'facebook':
				if (wp_is_mobile()) {
			...
			case 'twitter':
				...
				$twitter_via = (grandprix_mikado_options()->getOptionValue('twitter_via') !== '') ? esc_attr__( ' via ', 'grandprix-core' ) . grandprix_mikado_options()->getOptionValue('twitter_via') . ' ' : '';
