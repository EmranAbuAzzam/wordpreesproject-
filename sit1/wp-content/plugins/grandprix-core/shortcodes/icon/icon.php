<?php
namespace GrandPrixCore\CPT\Shortcodes\Icon;
...
class Icon implements Lib\ShortcodeInterface {
    ...
    public function vcMap() {
	    if(function_exists('vc_map')) {
		    vc_map(
		    	array(
				    'name'                      => esc_html__( 'Icon', 'grandprix-core' ),
				    'base'                      => $this->base,
				    ...
				    'params'                    => array_merge(
					    ...
					    grandprix_mikado_icon_collections()->getVCParamsArray(),
					    array(
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'size',
							    ...
							    'value'      => array(
								    esc_html__( 'Choose size', 'grandprix-core' ) => '',
								    esc_html__( 'Tiny', 'grandprix-core' )        => 'mkdf-icon-tiny',
								    esc_html__( 'Small', 'grandprix-core' )       => 'mkdf-icon-small',
								    esc_html__( 'Medium', 'grandprix-core' )      => 'mkdf-icon-medium',
								    esc_html__( 'Large', 'grandprix-core' )       => 'mkdf-icon-large',
								    esc_html__( 'Huge', 'grandprix-core' )        => 'mkdf-icon-huge'
							    )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'custom_size',
							    'heading'    => esc_html__( 'Custom Size (px)', 'grandprix-core' )
						    ),
						    array(
							    'type'       => 'dropdown',
							    'param_name' => 'type',
							    ...
							    'value'      => array(
								    esc_html__( 'Normal', 'grandprix-core' ) => 'mkdf-normal',
								    esc_html__( 'Circle', 'grandprix-core' ) => 'mkdf-circle',
								    esc_html__( 'Square', 'grandprix-core' ) => 'mkdf-square'
							    ),
							    'save_always' => true
						    ),
						    array(
							    ...
							    'heading'     => esc_html__( 'Border Radius', 'grandprix-core' ),
							    'description' => esc_html__( 'Please insert border radius(Rounded corners) in px. For example: 4 ', 'grandprix-core' ),
							    'dependency'  => array( 'element' => 'type', 'value' => array( 'mkdf-square' ) )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'shape_size',
							    'heading'    => esc_html__( 'Shape Size (px)', 'grandprix-core' ),
							    'dependency' => array( 'element' => 'type', 'value' => array( 'mkdf-circle', 'mkdf-square' ) )
						    ),
						    array(
							    'type'       => 'colorpicker',
							    'param_name' => 'icon_color',
							    'heading'    => esc_html__( 'Icon Color', 'grandprix-core' )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Border Width', 'grandprix-core' ),
							    'dependency' => array( 'element' => 'type', 'value'   => array( 'mkdf-circle', 'mkdf-square' ) )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Hover Icon Color', 'grandprix-core' )
						    ),
						    array(
							    'type'       => 'colorpicker',
						    ...
						    array(
							    ...
							    'heading'    => esc_html__( 'Icon Animation', 'grandprix-core' ),
							    'value'      => array_flip( grandprix_mikado_get_yes_no_select_array( false ) )
						    ),
						    array(
							    'type'       => 'textfield',
							    'param_name' => 'icon_animation_delay',
							    'heading'    => esc_html__( 'Icon Animation Delay (ms)', 'grandprix-core' ),
						    ...
						    array(
							    ...
							    'heading'     => esc_html__( 'Use Link as Anchor', 'grandprix-core' ),
							    'value'       => array( 'Use this icon as Anchor?' => 'yes' ),
							    'description' => esc_html__( 'Check this box to use icon as anchor link (eg. #contact)', 'grandprix-core' ),
	...
	public function render( $atts, $content = null ) {
		...
		$html = grandprix_core_get_shortcode_module_template_part( 'templates/icon', 'icon', '', $params );
	...
	private function generateIconStyles( $params ) {
		...
		if ( ( $params['type'] !== 'mkdf-normal' && ! empty( $params['shape_size'] ) ) || ( $params['type'] == 'mkdf-normal' ) ) {
			if ( ! empty( $params['custom_size'] ) ) {
				$iconStyles[] = 'font-size:' . grandprix_mikado_filter_px( $params['custom_size'] ) . 'px';
			}
		}
	...
	private function generateIconHolderStyles( $params ) {
		...
		if ( $params['type'] !== 'mkdf-normal' ) {
			...
			if ( ! empty( $shapeSize ) ) {
				$iconHolderStyles[] = 'width: ' . grandprix_mikado_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'height: ' . grandprix_mikado_filter_px( $shapeSize ) . 'px';
				$iconHolderStyles[] = 'line-height: ' . grandprix_mikado_filter_px( $shapeSize ) . 'px';
			}
			...
			if ( ! empty( $params['border_color'] ) && ( isset( $params['border_width'] ) && $params['border_width'] !== '' ) ) {
				...
				$iconHolderStyles[] = 'border-width: ' . grandprix_mikado_filter_px( $params['border_width'] ) . 'px';
			...
			if ( $params['type'] == 'mkdf-square' ) {
				if ( isset( $params['border_radius'] ) && $params['border_radius'] !== '' ) {
					$iconHolderStyles[] = 'border-radius: ' . grandprix_mikado_filter_px( $params['border_radius'] ) . 'px';
				}
			}
		}
