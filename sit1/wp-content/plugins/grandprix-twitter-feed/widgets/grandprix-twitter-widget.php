...
class GrandPrixTwitterWidget extends WP_Widget {
	private $params;
	
	public function __construct() {
		parent::__construct(
			'mkdf_twitter_widget',
			esc_html__( 'GrandPrix Twitter Widget', 'grandprix-twitter-feed' ),
			array(
				'description' => esc_html__( 'Display your Twitter feed', 'grandprix-twitter-feed' )
			)
		);
	...
	private function setParams() {
		$this->params = array(
			array(
				'name'  => 'title',
				'type'  => 'textfield',
				'title' => esc_html__( 'Title', 'grandprix-twitter-feed' )
			),
			array(
				'name'    => 'type_of_widget',
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Type', 'grandprix-twitter-feed' ),
				'options' => array(
					'standard' => esc_html__( 'Standard', 'grandprix-twitter-feed' ),
					'slider'   => esc_html__( 'Slider', 'grandprix-twitter-feed' )
				)
			),
			array(
				'name'  => 'user_id',
				'type'  => 'textfield',
				'title' => esc_html__( 'User ID', 'grandprix-twitter-feed' )
			),
			array(
				'name'  => 'count',
				'type'  => 'textfield',
				'title' => esc_html__( 'Number of Tweets', 'grandprix-twitter-feed' )
			),
			array(
				'name'    => 'show_tweet_icon',
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Show Tweet Icon', 'grandprix-twitter-feed' ),
				'options' => array(
					'yes' => esc_html__( 'Yes', 'grandprix-twitter-feed' ),
					'no'  => esc_html__( 'No', 'grandprix-twitter-feed' )
				)
			),
			array(
				'name'    => 'show_tweet_time',
				'type'    => 'dropdown',
				'title'   => esc_html__( 'Show Tweet Time', 'grandprix-twitter-feed' ),
				'options' => array(
					'no'  => esc_html__( 'No', 'grandprix-twitter-feed' ),
					'yes' => esc_html__( 'Yes', 'grandprix-twitter-feed' )
				)
			...
			array(
				'name'  => 'transient_time',
				'type'  => 'textfield',
				'title' => esc_html__( 'Tweets Cache Time', 'grandprix-twitter-feed' )
			)
		);
	}
	...
	public function widget( $args, $instance ) {
		...
		$twitter_api = GrandPrixTwitterApi::getInstance();
		...
		if ( $twitter_api->hasUserConnected() ) {
			...
			if ( $response->status ) {
				if ( is_array( $response->data ) && count( $response->data ) ) { ?>
					<ul class="mkdf-twitter-widget <?php echo esc_attr( $holder_classes ); ?>" <?php echo grandprix_mikado_get_inline_attrs( $slider_data ); ?>>
						<?php foreach ( $response->data as $tweet ) { ?>
							
							<?php if ( $type_of_widget == 'slider' ) { ?>
								
								<li class="mkdf-tweet-holder">
									<div class="mkdf-tweet-text" <?php grandprix_mikado_inline_style( $twitter_style ); ?>>
			...
			esc_html_e( 'It seams that you haven\'t connected with your Twitter account', 'grandprix-twitter-feed' );
		}
...
if ( ! function_exists( 'grandprix_twitter_widget_load' ) ) {
	function grandprix_twitter_widget_load() {
		if ( grandprix_twitter_theme_installed() ) {
			register_widget( 'GrandPrixTwitterWidget' );
		}
	}
	
	add_action( 'widgets_init', 'grandprix_twitter_widget_load' );
}